package nu.staldal.lagoon.producer;

import java.io.IOException;
import java.io.InterruptedIOException;
import org.junit.*;
import static org.junit.Assert.*;

/**
 * The class <code>MyInputStreamTest</code> contains tests for the class <code>{@link MyInputStream}</code>.
 *
 * @generatedBy CodePro at 4/23/14 7:56 PM
 * @author Jeshua
 * @version $Revision: 1.0 $
 */
public class MyInputStreamTest {
	/**
	 * Run the MyInputStream() constructor test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testMyInputStream_1()
		throws Exception {

		MyInputStream result = new MyInputStream();

		// add additional test code here
		// An unexpected exception was thrown in user code while executing this test:
		//    java.lang.UnsupportedClassVersionError: nu/staldal/lagoon/producer/MyInputStream : Unsupported major.minor version 51.0
		//       at java.lang.ClassLoader.defineClass1(Native Method)
		//       at java.lang.ClassLoader.defineClassCond(ClassLoader.java:637)
		//       at java.lang.ClassLoader.defineClass(ClassLoader.java:621)
		//       at java.security.SecureClassLoader.defineClass(SecureClassLoader.java:141)
		//       at java.net.URLClassLoader.defineClass(URLClassLoader.java:283)
		//       at java.net.URLClassLoader.access$000(URLClassLoader.java:58)
		//       at java.net.URLClassLoader$1.run(URLClassLoader.java:197)
		//       at java.security.AccessController.doPrivileged(Native Method)
		//       at java.net.URLClassLoader.findClass(URLClassLoader.java:190)
		//       at java.lang.ClassLoader.loadClass(ClassLoader.java:306)
		//       at com.instantiations.assist.eclipse.junit.execution.core.UserDefinedClassLoader.loadClass(UserDefinedClassLoader.java:62)
		//       at java.lang.ClassLoader.loadClass(ClassLoader.java:247)
		//       at com.instantiations.assist.eclipse.junit.execution.core.ExecutionContextImpl.getClass(ExecutionContextImpl.java:99)
		//       at com.instantiations.eclipse.analysis.expression.model.SimpleTypeExpression.execute(SimpleTypeExpression.java:205)
		//       at com.instantiations.eclipse.analysis.expression.model.InstanceCreationExpression.execute(InstanceCreationExpression.java:425)
		//       at com.instantiations.assist.eclipse.junit.execution.core.ExecutionRequest.execute(ExecutionRequest.java:286)
		//       at com.instantiations.assist.eclipse.junit.execution.communication.LocalExecutionClient$1.run(LocalExecutionClient.java:158)
		//       at java.lang.Thread.run(Thread.java:695)
		assertNotNull(result);
	}

	/**
	 * Run the int avaliable() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testAvaliable_1()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);

		int result = fixture.avaliable();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Run the int avaliable() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testAvaliable_2()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(-1);

		int result = fixture.avaliable();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Run the int avaliable() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testAvaliable_3()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);

		int result = fixture.avaliable();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Run the void close() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testClose_1()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);

		fixture.close();

		// add additional test code here
	}

	/**
	 * Run the void deliver(int) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testDeliver_1()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);
		int b = 1;

		fixture.deliver(b);

		// add additional test code here
	}

	/**
	 * Run the void deliver(int) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testDeliver_2()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);
		int b = 1;

		fixture.deliver(b);

		// add additional test code here
	}

	/**
	 * Run the void deliver(int) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testDeliver_3()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(-1);
		int b = 1;

		fixture.deliver(b);

		// add additional test code here
	}

	/**
	 * Run the void deliver(int) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test(expected = java.io.IOException.class)
	public void testDeliver_4()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);
		int b = 1;

		fixture.deliver(b);

		// add additional test code here
	}

	/**
	 * Run the void deliver(int) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test(expected = java.io.InterruptedIOException.class)
	public void testDeliver_5()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);
		int b = 1;

		fixture.deliver(b);

		// add additional test code here
	}

	/**
	 * Run the void eof() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testEof_1()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);

		fixture.eof();

		// add additional test code here
	}

	/**
	 * Run the int read() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testRead_1()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);

		int result = fixture.read();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Run the int read() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testRead_2()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(-1);

		int result = fixture.read();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Run the int read() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test
	public void testRead_3()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);

		int result = fixture.read();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Run the int read() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test(expected = java.io.IOException.class)
	public void testRead_4()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(1);

		int result = fixture.read();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Run the int read() method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Test(expected = java.io.InterruptedIOException.class)
	public void testRead_5()
		throws Exception {
		MyInputStream fixture = new MyInputStream();
		fixture.deliver(-1);

		int result = fixture.read();

		// add additional test code here
		assertEquals(0, result);
	}

	/**
	 * Perform pre-test initialization.
	 *
	 * @throws Exception
	 *         if the initialization fails for some reason
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@Before
	public void setUp()
		throws Exception {
		// add additional set up code here
	}

	/**
	 * Perform post-test clean-up.
	 *
	 * @throws Exception
	 *         if the clean-up fails for some reason
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	@After
	public void tearDown()
		throws Exception {
		// Add additional tear down code here
	}

	/**
	 * Launch the test.
	 *
	 * @param args the command line arguments
	 *
	 * @generatedBy CodePro at 4/23/14 7:56 PM
	 */
	public static void main(String[] args) {
		new org.junit.runner.JUnitCore().run(MyInputStreamTest.class);
	}
}