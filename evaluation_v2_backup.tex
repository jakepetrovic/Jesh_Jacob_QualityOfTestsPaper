...
%Execution time
The time to execute the generated test suites was also evaluated. Figure ~\ref{fig:TestExecTime}
...
In the study, we measured both branch coverage and mutation scores for each of the generation test suites.

%BranchCoverage%
We measured branch coverage for each of the generated test suites. Figure BRANCHCOVERAGE show manual test suites varied greatly in their coverage scores. However, manual test suites were the only to attain a branch coverage above 80\% wtih Lavalamp and Xisemele. However, Dieberse achieved a 0\% branch coverage for the manual tests. Overall CodePro had higher branch coverage scores than Evosuite or manual. Evosuite had a low margin of error at ERRORNUMBER.

%MutationScores%
We also measured the mutation score of each generated test suite in Figure MUTATIONSCORES. Evosuite overall retained the highest mutation scores for five of the ten programs. However, the margin of error for some programs were large. Dieberse produced a margin of error at ERRORNUMBER, which ranged the mutation scores between 18\% and 40\%. Manual test suites attained the highest score for four of the ten programs. CodePro acquired the highest mutation score from only Inspirento, and had the lowest score for the rest of the applications, even attaining a 0 mutation score for Lagoon, Saxpath, and Xisemele.

%Number of Tests vs Branch Coverage%
In Figures TESTVSBRANCHCOV1, 2, and 3, we examined the relationship between the number of tests and the branch coverage for each test generated suite. Figure MANUALTVSBRANCH retains the highest $R^2$ value, while the values for Evosuite and CodePro yield less fit results. Manual test suites increased in branch coverage as the number of test cases increased. Evosuite begins in a similar trend, but actually drops in branch coverage for the two larger test suites. Figure CODEPROTVSBRANCH indicates that CodePro branch coverage drops as the number of generated test cases increases. 

%LOC vs Branch Coverage%	
Our research evaluated the relationship between LOC and Branch coverage for each of the generated test suites. Figures MANUAL, CODEPRO, and EVOSUITE all acquired a low $R^2$ value for the best fit, but all displayed a similar trend in the decrease of Branch Coverage as the LOC of the application code increased. Only CodePro displays a larger increase in branch coverage for the largest program.
