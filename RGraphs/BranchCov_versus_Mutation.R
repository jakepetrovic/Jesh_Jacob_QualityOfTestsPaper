...
p2 <- p1 + theme_bw()
p3 <- p2 + labs(title="Mutation Score versus Branch Coverage- CodePro", x = "Branch Coverage (%)", y="Mutation Score (%)")
...
p2 <- p1 + theme_bw()
p3 <- p2 + labs(title="Mutation Score versus Branch Coverage- Manual", x = "Branch Coverage (%)", y="Mutation Score (%)")
...
p2 <- p1 + theme_bw()
p3 <- p2 + labs(title="Mutation Score versus Branch Coverage- Evosuite", x = "Branch Coverage (%)", y="Mutation Score (%)")
...
# Randoop ---------------------------------------------------------
p1 <- ggplot(data, aes(x=RBranchCov, y=RMutation))
p2 <- p1 + theme_bw()
p3 <- p2 + labs(title="Mutation Score versus Branch Coverage- Randoop", x = "Branch Coverage (%)", y="Mutation Score (%)")

p4_line <- p3 + geom_smooth(method="lm",formula = y~ x)
p4_poly <- p3 + geom_smooth(method="lm",formula = y~ poly(x,2))

lm_poly_eqn = function(df){
m=lm(RMutation ~ poly(RBranchCov, 2), df)#2nd degree polynomial
eq <- substitute(italic(y) == a + b %.% italic(x)* + c %.% italic(x)^2* ","~~italic(r)^2~"="~r2,
list(a = format(coef(m)[1], digits = 2),
b = format(coef(m)[2], digits = 2),
c = format(coef(m)[3], digits = 2),
...
p5_line + geom_point()
ggsave("Randoop_BranchCov_versus_Mutation_line.pdf")
p5_poly + geom_point()
ggsave("Randoop_BranchCov_versus_Mutation_poly.pdf")
