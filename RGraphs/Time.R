...
ggplot(data=dm.molten, aes(x=variable, y=value, fill=Program)) + geom_bar(stat="identity", position=position_dodge())+ theme_minimal()+labs(title="Time of Test Case Generation", x = "Programs", y="Time to Generate (s)")+guides(fill=guide_legend(title="Generation Tools"))+ 
